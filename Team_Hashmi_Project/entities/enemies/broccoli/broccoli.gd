...
var BROC_PROJ = preload("res://entities/enemies/broccoli/broc-proj.tscn")
#region Animations
const ATTACK = "attack"
const DAMAGE = "damage"
const IDLE = "idle" 
...
var MOVEMENT_SPEED_HANDICAP = 25
...
func _ready() -> void:
    ...
    if health == 9999:
        ...
        type = EntityTypeEnums.ENEMY_TYPE.BROCCOLI
...
func player_location_changed(_position: Vector2):
    ...
    if (
        abs(dist) < WALKING_DISTANCE * 1.5 and
...
func _on_Image_frame_changed() -> void:
    if (
        ...
        var newProj = BROC_PROJ.instance()
        newProj.init(Vector2(position.x, position.y), Vector2(5 * lastDir,0) )
        lastDir = null
        get_tree().get_root().add_child(newProj)
